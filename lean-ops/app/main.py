...
# 知识库（从 webapp 迁入，注册于页面路由之后）
from app.api.v1.knowledge import setup_kb_routes  # noqa: E402
setup_kb_routes(app, templates)

# ==================== Phase 2: webapp 独有页面迁入 ====================
@app.get("/lean20", response_class=HTMLResponse, include_in_schema=False)
async def lean20_page(request: Request):
    return templates.TemplateResponse("lean20.html", {"request": request})

@app.get("/lean20-assess", response_class=HTMLResponse, include_in_schema=False)
async def lean20_assess_page(request: Request):
    return templates.TemplateResponse("lean20_assess.html", {"request": request})

@app.get("/pillars", response_class=HTMLResponse, include_in_schema=False)
async def pillars_page(request: Request):
    return templates.TemplateResponse("pillars.html", {"request": request})
...
async def assessment_page(request: Request):
    from app.api.v1.knowledge import scan_directory_tree, simplify_module, MODULES
    ...
    return templates.TemplateResponse("assessment.html", {"request": request, "module": module})

@app.get("/implementation", response_class=HTMLResponse, include_in_schema=False)
async def implementation_page(request: Request):
    ...
    return templates.TemplateResponse("implementation.html", {"request": request, "impl_module": impl_module, "project_module": project_module})
